    ...
    [:div.container
     ...
     [:ul
      ...
      [:li [:p "Start making your webapp!"]
       [:ul
        [:li [:p
              "The CLJS entry point is in " [code "ui-src/word_finder/ui/app.cljs"] ". "
              "Figwheel should auto-reload any connected browsers on every save."]]
        [:li [:p "The Clojure system entry point is in " [code "src/word_finder/service/main.clj"]]]
        [:li [:p "The Clojure Ring handler is in " [code "src/word_finder/service/handler.clj"]]]]]
